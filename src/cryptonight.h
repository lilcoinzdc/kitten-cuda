...
#if defined(kitten_ALGO_KAWPOW) || defined(kitten_ALGO_CN_R)
#   include <cuda.h>
#endif
...
#   endif
    ...
    kitten_cuda::Algorithm algorithm     = kitten_cuda::Algorithm::INVALID;
...
void cryptonight_extra_cpu_prepare(nvid_ctx *ctx, uint32_t startNonce, const kitten_cuda::Algorithm &algorithm);
void cryptonight_gpu_hash(nvid_ctx *ctx, const kitten_cuda::Algorithm &algorithm, uint64_t height, uint32_t startNonce);
void cryptonight_extra_cpu_final(nvid_ctx *ctx, uint32_t startNonce, uint64_t target, uint32_t *rescount, uint32_t *resnonce, const kitten_cuda::Algorithm &algorithm);

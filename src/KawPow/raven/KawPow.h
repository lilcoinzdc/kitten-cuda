...
DEV_INLINE void progPowLoop(const uint32_t loop, uint32_t mix[PROGPOW_REGS], const dag_t *g_dag, const uint32_t c_dag[PROGPOW_CACHE_WORDS], const bool hack_false)
{
    ...
    kitten_INCLUDE_OFFSET_MOD_DAG_ELEMENTS
    ...
    kitten_INCLUDE_PROGPOW_RANDOM_MATH

    // consume global load data
    // hack to prevent compiler from reordering LD and usage
    if (hack_false) __threadfence_block();

    kitten_INCLUDE_PROGPOW_DATA_LOADS
}
...
__global__ void kitten_INCLUDE_LAUNCH_BOUNDS progpow_search(const dag_t *g_dag, const uint32_t* job_blob, const uint64_t target, bool hack_false, uint32_t* results, uint32_t* stop)
{
    if (*stop) {
